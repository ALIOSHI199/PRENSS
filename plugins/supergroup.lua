...
if result.username then
 channel_username = "☑يوزر نيم المجموعه: @"..result.username
else
 channel_username = ""
end
...
local function lock_group_links(msg, data, target)
  ...
  local group_link_lock = data[tostring(target)]['settings']['lock_ads']
  if group_link_lock == 'yes' then
    return 'الروابط بالفعل☑️تم قفلها🔒\n🔺الامرني🎃 : @'..msg.from.username..'\n🔻ايديه : '.. msg.from.id..'\n'
  else
    data[tostring(target)]['settings']['lock_ads'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل 🎃🙅🏼‍♂️ ◻ (  الروابط  )◼ ❣الامر تم تنفيذه بنجاح يا ◙ سيدي ◙ 😝😍   ↫♚ الطلب مني ❧\nالامرني : @'..msg.from.username..'\nايديه : '.. msg.from.id..'\n'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم فتح 🎃🙅🏼‍♂️ ◻ (  الروابط  )◼ ❣الامر تم تنفيذه بنجاح يا ◙ سيدي ◙ 😝😍   ↫♚ الطلب مني ❧  \nالامرني  : @'..msg.from.username..'\nايديه : '.. msg.from.id..'\n'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "📛 لا يمكنك التحكم ⚠️ فقط الادمنيه يمكنهم🌝"
  end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'لــم يتــم وضــع قوانين في مجموعه :- '..msg.to.title..'\n'
  end
...
local function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return " لا يمكنك التحكم في البوت للادمنيه فقط"
  end
...
local function promote_admin(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..'هوه ضمن الادمنيه🌝')
  end
...
local function demote_admin(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..'هوه ضمن الاعضاء🌝')
  end
...
local function promote2(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..'هوه️ ضمن الادمنيه🌝')
  end
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..'وَيَنَ الُمٌْعزُوَفَُه 💃يَابّ تْمٌ رَفَْعكِ ادِمٌنَ😉')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
     return send_large_msg(receiver, member_username..'طِفَيَ الُمٌْعزُوَفَُه ليَنَ انَت فَاشِلُ تْمٌ تْنَزُيَلُكِ مٌنَ الُاِمٌنَ 😂')
  end
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..'طِفَيَ الُمٌْعزُوَفَُه ليَنَ انَت فَاشِلُ تْمٌ تْنَزُيَلُكِ مٌنَ الُاِمٌنَ 😂')
end
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
               return send_large_msg("channel#id"..channel_id, " لا يمكنك طرد الادمن ❗️ او المدير")
    end
    if is_admin2(member_id) then
         return send_large_msg("channel#id"..channel_id, "🌝 لا يمكنك️ طرد الاداري ")
    end
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
               return send_large_msg("channel#id"..channel_id, " لا يمكنك️ طرد الادمن ❗️ او المدير")
    end
    if is_admin2(member_id) then
    ...
    elseif get_cmd == "رفع اداري" then
        ...
        if result.from.username then
            text = "@"..result.from.username.."تم رفعك اداري حبيبي🌝"
        else
            text = "[ "..user_id.." ] تم رفعك اداري حبيبي🌝 "
        end
    ...
    elseif get_cmd == "تنزيل اداري" then
        ...
        if result.from.username then
            text = "@"..result.from.username.."تم ️تنزيلك️ من الاداره  "
        else
            text = "[ "..user_id.." ] تم️ تنزيلك️ من الاداره"
        end
    ...
    elseif get_cmd == "رفع المدير" then
        ...
        local channel_id = 'channel#id'..result.to.peer_id
            ...
            if result.from.username then
                text = "@"..result.from.username.." [ "..result.from.peer_id.." ]   تْمٌ رَفَْعكِ مٌدِيَرَ 🕵الُمٌجْمٌوَْعُه رَوَحُ جْيَبّلُكِ مٌطِرَبّ ٌخلُيَ نَرَكِصّ 💃👯 🌝  "
            else
                text = "[ "..result.from.peer_id.." ]   تْمٌ رَفَْعكِ مٌدِيَرَ 🕵الُمٌجْمٌوَْعُه رَوَحُ جْيَبّلُكِ مٌطِرَبّ ٌخلُيَ نَرَكِصّ 💃👯🌝  "
            end
            send_large_msg(channel_id, text)
        end
...
local function cb_user_info(extra, success, result)
    ...
    if get_cmd == "تنزيل اداري" then
        ...
        if result.username then
            text = "@"..result.username.."تم تنزيلك من الاداره🌝"
            send_large_msg(receiver, text)
        else
            text = "[ "..result.peer_id.." ]تم تنزيلك من الاداره🌝"
            send_large_msg(receiver, text)
        end
    elseif get_cmd == "رفع ادمن" then
        if result.username then
...
elseif get_cmd == "رفع اداري" then
   for k,v in pairs(result) do
    ...
    if vusername == member or vpeer_id == memberid then
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."]تم رفعك في الاداره حبيبي🌝"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "["..v.peer_id.."]تم رفعك في الاداره حبيبي🌝"
    ...
    for k,v in pairs(result) do
        ...
        if vusername == member or vpeer_id == memberid then
            ...
            if group_owner then
                ...
                if result.username then
                    text = member_username.." ["..v.peer_id.."] تْمٌ رَفَْعكِ مٌدِيَرَ 🕵الُمٌجْمٌوَْعُه رَوَحُ جْيَبّلُكِ مٌطِرَبّ ٌخلُيَ نَرَكِصّ 💃👯"
                else
                    text = "["..v.peer_id.."] تْمٌ رَفَْعكِ مٌدِيَرَ 🕵الُمٌجْمٌوَْعُه رَوَحُ جْيَبّلُكِ مٌطِرَبّ ٌخلُيَ نَرَكِصّ 💃👯"
                end
            end
        elseif memberid and vusername ~= member and vpeer_id ~= memberid then
            ...
            if group_owner then
                ...
                text = "["..memberid.."] تْمٌ رَفَْعكِ مٌدِيَرَ 🕵الُمٌجْمٌوَْعُه رَوَحُ جْيَبّلُكِ مٌطِرَبّ ٌخلُيَ نَرَكِصّ 💃👯"
            end
        end
    end
 end
    ...
    local data = load_data(_config.moderation.data)
        ...
        if matches[1] == "ضع معرف" and is_admin1(msg) then
            local function ok_username_cb (extra, success, result)
                local receiver = extra.receiver
                if success == 1 then
                    send_large_msg(receiver, "تم وضع معرف 🆔 للمجموعه 👍😽\n\n🌐 انظر الى الحول لتشاهد تغيرات 👍 المجموعه 👥😽")
        ...
        if matches[1] == 'قفل' and is_momod(msg) then
            ...
            if matches[2] == 'الروابط' then
                savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked lock_ads ")
        ...
        if matches[1] == 'فتح' and is_momod(msg) then
            local target = msg.to.id
            if matches[2] == 'الروابط' then
                savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked lock_ads")
        ...
        if matches[1] == 'ضع تكرار' then
            ...
            if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 20 then
                return " ضع تكرار من 5🔺🔻 الى"
            end
            local flood_max = matches[2]
            data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
            save_data(_config.moderation.data, data)
            savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
            return 'تم️ تعيين التكرار ‼️⚙ للعدد 👈🏿: '..matches[2]
                    end
